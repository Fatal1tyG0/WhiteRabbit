...
> После заполнения вышеуказанных сведений "Остатки товара" из компонента `InventoryStock` приобретают свойства компонента `MyGoods` и дальнейшая работа ведется именно по этому компоненту. Пример: пользователь отправил на "Приёмку грузов" товар (объект) со следующими свойствами: 1) Название - кроссовки, 2) Количество - 1000 шт, 3) Количество коробок - 20, 4) Вес груза - 1000 кг, 5) Объём груза - 2,8 м3. Данная приёмка прошла согласование менеджером и была получена на складе. У пользователя отображается на остатках товара на "Склад Москва" не обличенный товар с этими характеристиками. После этого пользователь должен обличить его, для чего вводит свойства для компонента `MyGoods`. При их заполнении товар приобретает свойства артикула и считается обличенным, после чего с ним можно производить полноценные работы и передавать в компонент 
---

<!--stackedit_data:
eyJoaXN0b3J5IjpbNTk5NTAyNTgzLC0zNTY0MzQ5MTQsMTM2NT
EwOTIzMl19
-->